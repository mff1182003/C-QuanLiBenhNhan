...
namespace QuanlyBenhnhan
{
    public partial class PhongTrong : Form
    {
        ...
        private void btnCapgiuong_Click(object sender, EventArgs e)
        {
            this.Hide();
            CapGiuong capGiuong = new CapGiuong();
            capGiuong.setIDgiuong(GrvGiuong.SelectedCells[0].Value.ToString());
            capGiuong.ShowDialog();
        }
        public void Khoa()
        {
            try
        ...
        private void cbPhong_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string lenh = @"SELECT Giuong.IDgiuong, Khoabenh.Tenkhoa, Phong.Tenphong, Giuong.Tengiuong, Giuong.Status
                    FROM     Phong INNER JOIN
                  Khoabenh ON Phong.IDkhoa = Khoabenh.IDkhoa INNER JOIN
                  Giuong ON Phong.IDphong = Giuong.IDphong
                    WHERE  (Giuong.Status = N'Trống')  AND (Phong.Tenphong = N'" + cbPhong.Text + "') And (Khoabenh.Tenkhoa=N'" + cbKhoa.Text + "')";
                if (conn.State != ConnectionState.Open)
                {
                    conn.Open();
                }
